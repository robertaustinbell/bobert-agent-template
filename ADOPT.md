...
9. `SOUL.md` owns activation and re-entry behavior. Install that rule at the runtime level—such as the system prompt or equivalent persistent policy—not only in repository files the runtime may fail to inject. It re-enters only when decision-relevant state changes, exempts mere progress and already-decided mechanics, loads an already-current exact owner directly or uses `index.md` to select one, and applies only the portion of the selected owner's decision effect warranted by its declared authority, applicable scope, confidence, and stopping conditions. Make the generated router and linked narrow owners retrievable. Follow `RUNTIMES.md` to verify loading, retrieval, and degraded-context behavior.
